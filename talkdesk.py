...
def authenticate():
    ...
    if r.status_code != 200:
        logging.info('Bad status code on authenticate: %d' % r.status_code)
        exit()
...
def create_report_job():
    ...
    logging.info('Creating report for calls from %s to %s' % (from_time, to_time))
    ...
    if r.status_code != 202:
        logging.info('Bad status code: %d' % r.status_code)
        exit()
    return r.json()

if __name__ == '__main__':
    auth_header = authenticate()
    logging.info('Successfully authenticated to API.')
    report_job = create_report_job()
    logging.info('Created report with id ' + report_job['id'])
    ...
    while True:
        ...
        if report.status_code == 200:
            ...
            break
        logging.info('Report not ready yet, sleeping...')
        time.sleep(10)
    ...
    if login_response.status_code != 200:
        logging.info('Error logging into Talkdesk to download recordings.')
        exit()
    logging.info('Successfully logged in for downloading call mp3s.')
    ...
    for entry in report['entries']:
        if entry['agent_name'] in DOWNLOAD_REP_LIST and MIN_CALL_TIME <= entry['total_time'] <= MAX_CALL_TIME:
            if entry['recording_url'] is not None:
                logging.info('Downloading call for %s from %s' % (entry['agent_name'], entry['recording_url']))
                r = login_session.get(entry['recording_url'], stream=True)
                with open('%s_%s.mp3' % (entry['agent_name'], entry['callsid']), 'wb') as fd:
                    for chunk in r.iter_content(chunk_size=1024):
                        fd.write(chunk)
            else:
                logging.info('recording_url for %s was None' % entry['agent_name'])
